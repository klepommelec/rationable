...
export const VisualIndicators: React.FC<VisualIndicatorsProps> = ({
  ...
  // Calculer les statistiques des pros/cons avec validation des arrays
  const totalPros = data.reduce((acc, item) => {
    const pros = Array.isArray(item.pros) ? item.pros : [];
    return acc + pros.length;
  }, 0);
  
  const totalCons = data.reduce((acc, item) => {
    const cons = Array.isArray(item.cons) ? item.cons : [];
    return acc + cons.length;
  }, 0);
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {prosConsMetrics.map((metric, index) => (
              <div key={metric.title} className={`p-4 rounded-lg ${metric.color} animate-fade-in`} style={{
                ...
                <div key={item.option} style={{
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div className="text-green-700 bg-green-50 p-2 rounded">
                      <div className="font-medium mb-1">Points forts ({pros.length})</div>
                    ...
                    <div className="text-red-700 bg-red-50 p-2 rounded">
                      <div className="font-medium mb-1">Points faibles ({cons.length})</div>
                      {cons.slice(0, 2).map((con, i) => (
                        <div key={i} className="text-xs text-black py-[0.5px]">• {con}</div>
                      ))}
                    </div>
                  </div>
                </div>
