...
export const VisualIndicators: React.FC<VisualIndicatorsProps> = ({
  ...
  return <div className="space-y-6">
      ...
      {/* Analyse Pros/Cons */}
      <div>
        <h4 className="text-sm font-medium text-gray-500 mb-3">Analyse option recommandée par l'IA</h4>
      ...
      {/* Analyse détaillée */}
      <div>
        <h4 className="text-sm font-medium text-gray-500 mb-3">Analyse détaillée</h4>
        <div className="space-y-4">
          {sortedData.map((item, index) => {
          const cleanName = item.option.replace(/^Option\s+\d+:\s*/i, '').trim();
          return <div key={item.option} style={{
            animationDelay: `${index * 100}ms`
          }} className="space-y-2 p-3 rounded-lg border transition-colors animate-fade-in bg-background">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="text-green-700 bg-green-50 p-2 rounded">
                    ...
                    {item.pros.map((pro, i) => <div key={i} className="text-xs text-black py-[0.5px]">• {pro}</div>)}
                  </div>
                  <div className="text-red-700 bg-red-50 p-2 rounded">
                    <div className="font-medium mb-1">Points faibles ({item.cons.length})</div>
                    {item.cons.map((con, i) => <div key={i} className="text-xs text-black py-[0.5px]">• {con}</div>)}
                  </div>
                </div>
              </div>;
        })}
